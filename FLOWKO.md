...
- **U9 Tokens encrypted at rest**: Google's OAuth policy requires OAuth tokens encrypted at rest. Upstream kept the token object in plaintext in `Credential.key`. Its optional keyring copy in `encryptedKey` swallowed encryption errors, and its only decrypting reader fell back to the plaintext key.
  ...
  - *Where tokens are decrypted:* only in `CalendarAuth` (lazily, when a Google call needs them) and at the revoke and account-lookup sites of `handleDeleteCredential` and `replaceEarlierCredentials`. The code is in `packages/features/credentials/services/CredentialDataService.ts` and `packages/lib/crypto/keyring.ts`. The refresh write-back re-encrypts with the CURRENT kid (`CredentialRepository.updateEncryptedKeyWhereId`, only while the row keeps its type, userId and teamId), and no token is copied back into an in-memory `credential.key`. The fail-open decrypt in `getCalendarsEvents` is gone. `updateTokenObjectInDb` also serves the disabled pipedrive app, which would now store an encrypted key its own reader can't use.
  - *Fail closed:* every failure raises `CredentialKeyUnavailableError` with a reason: `keyring_not_configured` or `decrypt_failed` (transient), `not_encrypted` or `malformed_envelope` (permanent). It is logged once per credential and reason every 5 minutes as `Credential key unavailable (<reason>) for credential <id|new> (<type>)`, never with a token, an envelope field, an env value or an error object. A key failure never marks a credential invalid.
    - Connect: `add` answers 503 before Google's consent screen, and the callback answers 503 before the code exchange. If storing fails after the exchange, the fresh token is revoked and no row is created.
    - Availability: that host's whole range is busy, so no slot is offered; other tenants are unaffected. Where a booking still goes through (a create, reschedule or cancel), its Google event is missing or stale and the failure is logged.
    - Refresh: nothing is written, and the refreshed access token is discarded.
    - Disconnect: refused ("try again later") while the failure is transient, so the Google grant is still revoked once the key is back. With a permanent failure the row is deleted without a revoke, and the credential id and reason are logged.
  ...
  - *DB backstop:* migration `20260925120000_flowko_credential_no_plaintext_oauth_tokens` adds the validated CHECK `Credential_key_no_plaintext_oauth_tokens`, `"key"::text !~ '(access_token|refresh_token|id_token)'`. An INSERT or UPDATE that puts a token field, or any name or value containing those words, into `key` fails with SQLSTATE 23514, whatever the code path. The writers that now fail there are all off on this instance: other apps' OAuth callbacks and refreshes, `markTokenAsExpired` (credential sync), the app-credential webhook, delegation write-backs, the dev seed's Google fixture (`E2E_TEST_CALCOM_QA_GCAL_CREDENTIALS`) and upstream E2E fixtures that store tokens in `key`. **Enabling any other OAuth app, Google or Outlook login, credential sync or delegation first needs U9 extended to it.**
  ...
  - *Verification* (prints no secrets):
    - Before the deploy, `SELECT count(*) FROM "Credential" WHERE "key"::text ~ '(access_token|refresh_token|id_token)';` must return 0, or the migration fails.
    - After the deploy, the CHECK must exist and be validated. This check is mandatory: `scripts/start.sh` has no `set -e`, so a failed migration still boots the app, without the CHECK, and blocks every later migration (P3009).
      ```sql
      ...
      WHERE conrelid = '"Credential"'::regclass AND conname = 'Credential_key_no_plaintext_oauth_tokens';
      ```
    ...
    - Standing invariants, both 0 after any connect or refresh:
      ```sql
      SELECT count(*) FILTER (WHERE type = 'google_calendar' AND "encryptedKey" IS NULL) AS google_rows_without_envelope,
             count(*) FILTER (WHERE "key"::text ~ '(access_token|refresh_token|id_token)') AS rows_with_token_fields
      FROM "Credential";
      ```
  - *Rollback:* an image without U9 can't parse the placeholder, so after a Google connect a rollback breaks Google for those tenants (nothing leaks), and its plaintext writes hit the CHECK. Roll forward instead. If a rollback can't be avoided, first disconnect every Google credential with the U9 image (it revokes through decryption), then drop the CHECK by hand.
- **U9j branding leftovers**: see the U9j commit.
