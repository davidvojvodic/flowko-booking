...
- **U8b Booker wording and cosmetics** (*planned*):
  ...
  - Every way of connecting Google Calendar (the Install and Connect buttons of the app store, onboarding and the calendar settings, the calendar settings' Add menu and the app installation's account step) first shows a notice of what Flowko reads and writes in the calendar, linking the privacy policy's section 3.3, and goes on to Google's consent screen only after Continue. Google's Workspace API user data policy asks for this disclosure right before the consent request. The notice (`google_calendar_connect_notice`, English and Slovenian) names Flowko directly, because the Slovenian text declines the name, so it does not follow `NEXT_PUBLIC_APP_NAME`.
