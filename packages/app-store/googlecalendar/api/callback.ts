...
async function getHandler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (code) {
    ...
    try {
      ...
      // another live connection, the user's own or another user's, is the same Google account, or when
      // Google can't name the fresh token's account. The new credential itself still holds this very token,
      // so it is excluded: it would confirm its own grant as shared. Upstream deleted the credential and left
      // the grant live at Google. Best effort: it never throws, so the credential is still deleted and the
      // host is still sent back. (account_already_linked can't happen with today's SelectedCalendar upsert:
      // it moves the host's user-level row to the new credential, and a NULL eventTypeId never trips the
      // unique index. If it did, the row's holder would normally be an earlier connection to the same
      // account.)
