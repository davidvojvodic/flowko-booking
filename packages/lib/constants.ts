...
// Flowko: without the build arg this fell back to help@cal.com (U11)
export const SUPPORT_MAIL_ADDRESS = process.env.NEXT_PUBLIC_SUPPORT_MAIL_ADDRESS || "info@flowko.io";
...
// Dynamic group links (/userA+userB) are off on this instance, whatever User.allowDynamicBooking says
export const IS_DYNAMIC_GROUP_BOOKING_ENABLED = false;
// Seated (seatsPerTimeSlot) and recurring (recurringEvent) event types are off on this instance: the event
// type handlers refuse to set either and the booking services refuse to book one. true restores upstream.
export const IS_SEATS_AND_RECURRING_ENABLED = false;
// Needed for orgs
...
// Flowko: without the NEXT_PUBLIC_ build args these fell back to Cal.com's policies; the Google Calendar
// connect notice, the booking form and signup link them, so fall back to Flowko's own.
export const WEBSITE_PRIVACY_POLICY_URL =
  process.env.NEXT_PUBLIC_WEBSITE_PRIVACY_POLICY_URL || "https://flowko.si/privacy";
export const WEBSITE_TERMS_URL = process.env.NEXT_PUBLIC_WEBSITE_TERMS_URL || "https://flowko.si/terms";
